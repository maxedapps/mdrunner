...
| T05 | T1.3: add human-curated Keep-a-Changelog structure and release comparison links | T01 | In progress | Heading/category/link inspection and tagged dist plan | Unreleased/categories/comment/links verified; final dated 0.1.0 extraction intentionally joins T14 after hosted statuses |
...
| T09 | T2.2: add read-only native Linux/macOS/Windows CI for fmt/clippy/tests/release help/version smoke without publication/browser invocation | T04,T07 | In progress | Workflow syntax/matrix/manual inspection and hosted checks when available | Repository-owned CI parses; read-only token; bounded Linux quality and Linux/macOS/Windows locked test/release/smoke jobs; hosted execution joins T12 |
...
| T12 | T3.1: run one non-publishing setup-PR upload, inspect all hosted artifacts/checksums/linkage, then restore plan mode and regenerate | T08-T11 | In progress | Hosted run URLs/inventory plus final plan-mode diff and checks | Local upload generation proved PR build jobs and publication-gated host, then restored stable plan mode; remote branch/PR authorization and hosted run remain H02 |
| T13 | T3.2: record build-tested versus native-qualified evidence for each target and Linux baseline without unsupported claims | T12 | In progress | Per-target build/package/checksum and native-desktop evidence matrix | README/PROJECT/guide truthfully show Mac arm native-qualified, all hosted states pending and other targets unqualified; final per-target/linkage evidence waits on T12 |
| T14 | T3.3: prepare exact `v0.1.0` changelog/tag/release handoff, equality checks, unsigned caveats, approval gate, and recovery; do not publish | T12,T13 | Pending | Clean-candidate release-guide run and ready-to-tag commands; no tag/release | Publication remains owner-gated |
| T15 | Final validation/reconciliation: run all local/hosted/manual gates, inspect complete diff/residue, and complete independent boundary/final reviews | T01-T14 | Pending | All applicable gates pass; every finding disposed; no pending/blocking row | — |
...
| B04 / T12-T14 | Parent ownership: remote repository/CI credentials, sequential upload→inspect→restore flow, status/doc integration, and publication gates are unsafe to delegate; user approved H02 | Hosted non-publishing proof, qualification evidence, ready-to-tag handoff after verified Phase 2 | Parent-controlled branch/PR only; no tag/publication; local upload-mode simulation restored to plan before remote work | Hosted native/upload evidence, artifact inventory, qualification update, fresh Phase 3 review | User approved branch push/setup PR/temporary upload/final plan restoration on 2026-07-20; execution in progress |
| B05 / T15 | Parent integration ownership (full-plan reconciliation cannot be safely partitioned) | Whole diff, all evidence, final cleanup and gate | Sequential read-only final reviewer; tracker remains parent-only | Fresh full plan-backed review | — |
...
|---|---|---|---|---|---|
| H01 / T3.3 | Whether to create/push `v0.1.0` and publish the first GitHub Release | Plan explicitly prohibits publication without owner approval | Irreversible public/possibly immutable release | Prepare handoff only during implementation | Open; approval not provided |
