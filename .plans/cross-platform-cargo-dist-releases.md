...
| T09 | T2.2: add read-only native Linux/macOS/Windows CI for fmt/clippy/tests/release help/version smoke without publication/browser invocation | T04,T07 | In progress | Workflow syntax/matrix/manual inspection and hosted checks when available | Run 29732047255 exposed two Unix-literal unit fixtures; fixed. Final run 29732813090 then passed unit/CLI tests on Windows but exposed verbatim-canonical Windows containment mismatch in representative relative links. Parent normalized containment through file-URL roundtrip and made canonical-path unit coverage; fmt/35 tests/clippy pass locally; hosted rerun pending |
...
| T14 | T3.3: prepare exact `v0.1.0` changelog/tag/release handoff, equality checks, unsigned caveats, approval gate, and recovery; do not publish | T12,T13 | In progress | Clean-candidate release-guide run and ready-to-tag commands; no tag/release | Dated changelog and exact extracted title/body/installers/status/caveats verified; annotated tag/recovery/approval steps documented; final PR-head/equality gate and clean ready-to-tag commit pending |
...
| B04 / T12-T14 | Parent ownership: remote credentials, sequential upload→inspect→restore flow, status/doc integration, and publication gates are unsafe to delegate; user approved H02 | Hosted non-publishing proof, qualification evidence, ready-to-tag handoff after verified Phase 2 | Parent branch `chore/cross-platform-cargo-dist-releases`, PR #1; no tag/publication | Hosted native/upload evidence, artifact inventory, qualification update, fresh Phase 3 review | Run 29732047224 passed cargo-dist and artifacts. Native CI iteratively exposed (1) Unix-only test literals and (2) real Windows canonical-path/file-URL containment mismatch; parent-owned due immediate tightly coupled hosted diagnosis. Both fixes are narrow, locally gated; next hosted rerun pending. Redundant upload canceled; plan restored |
