...
| T3.1 | Phase 3: native release file/stdin smoke outside repo and agent-browser `file://` inspection; record only tested target | Phase 2 | In progress | retained HTML paths, prompt exit, visual/network/console evidence, binary hash/size | Parent-owned: actual local browser/process evidence requires current checkout and interactive agent-browser; not suitable for isolated writer. |
...
| B03 / T3.1–T3.2b | Parent owns T3.1 manual browser/process smoke because it requires the real local default-browser flow; isolated cleanup/docs writer considered only after smoke | Validate release before destructive legacy removal, then cut over | Sequential; no legacy deletion before T3.1 | Release/manual gate and cutover review | T3.1 in progress |
