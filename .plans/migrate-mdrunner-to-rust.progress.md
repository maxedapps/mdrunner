...
| T3.2a | Phase 3: remove Bun/TypeScript/N-API runtime, build/config/tests, obsolete artifacts/selectors while retaining fixtures/history | T3.1 | In progress | legacy/dependency search; intentional Git inventory | Isolated cleanup/docs writer after T3.1 gate; owns deletions/config/docs only, no Rust behavior changes. |
| T3.2b | Phase 3: update README/PROJECT/.gitignore for Rust workflow, behavior, cache/browser/no-server policy and tested target | T3.1 | In progress | documentation inspection plus final checks | Same isolated cutover writer; document only natively tested macOS arm64 target. |
...
| B03 / T3.1–T3.2b | Parent owned T3.1 real browser/process smoke; isolated writer owns now-safe legacy cleanup/docs after release evidence | T3.1 passed before destructive T3.2 cutover | Sequential; cleanup writer owns legacy deletions, README/PROJECT/.gitignore only; parent retains Rust implementation/tracker | Release/manual gate, final Rust checks, legacy search, M03 | T3.1 verified on macOS arm64; T3.2 writer pending launch |
