...
- `.github/workflows/ci.yml` is repository-owned and independently runs project tests and native release-binary smoke checks.
...
1. Require ordinary native CI to pass or run in parallel.
...
6. Return `pr-run-mode` to `"plan"`, rerun `dist init --yes` and `dist generate --check`, push the restoration, and require final native CI plus release planning to pass before merge.
